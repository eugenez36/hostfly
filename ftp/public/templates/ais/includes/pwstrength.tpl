...
jQuery("#inputNewPassword1").keyup(function(e) {
    var textBlock = $(this).parents('.form-group').find('.validation-text');
    textBlock.show();
    var messLang = textBlock.find('[data-pass="lang"]');
    var messLeng = textBlock.find('[data-pass="length"]');
    var messNum = textBlock.find('[data-pass="num"]');
    var messUpper = textBlock.find('[data-pass="upper"]');
...
{/if}
    ...
    if (pw.length < 8 || (numeric < 1 && numsymbols < 1) || upper < 1) {
        ...
        if (password2 !== '') {
            ...
            {if !isset($noDisable)}
                if (submitSuccess === true) {
                    $('input[type="submit"]').removeAttr('disabled').prop('disabled', false);
                }
            {/if}
        } else {
