...
{if $twoFactorAuthAvailable}
    <div class="card">
        <div class="card-body">
            ...
            <p class="twofa-config-link disable{if !$twoFactorAuthEnabled} w-hidden hidden{/if}">
                {lang key='twofacurrently'} <strong>{lang key='enabled'|strtolower}</strong>
            </p>
            <p class="twofa-config-link enable{if $twoFactorAuthEnabled} w-hidden hidden{/if}">
