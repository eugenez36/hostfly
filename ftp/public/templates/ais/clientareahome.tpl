<div class="tiles clearfix">
    <div class="row">
        <div class="col-sm-3 col-xs-6 tile tile_services" onclick="window.location='clientarea.php?action=services'">
            <a href="clientarea.php?action=services">
                <div class="icon">
                    <img class="icon-img" src="templates/ais/img/tile_services.svg" alt="Icon">
                    <i class="svg svg_reload-pink"></i>
                </div>
        ...
        {if $registerdomainenabled || $transferdomainenabled}
            <div class="col-sm-3 col-xs-6 tile tile_domains" onclick="window.location='clientarea.php?action=domains'">
                <a href="clientarea.php?action=domains">
                    <div class="icon">
                        <img class="icon-img" src="templates/ais/img/tile_domains.svg" alt="Icon">
                        <i class="svg svg_ok-green"></i>
                    </div>
        ...
        {/if}
        <div class="col-sm-3 col-xs-6 tile tile_tickets" onclick="window.location='supporttickets.php'">
            <a href="supporttickets.php">
                <div class="icon">
                    <img class="icon-img" src="templates/ais/img/tile_tickets.svg" alt="Icon">
                    <i class="svg svg_settings-orange"></i>
                </div>
                <div class="stat">{$clientsstats.numactivetickets}</div>
                <div class="title">{$LANG.navtickets}</div>
            </a>
        </div>
        <div class="col-sm-3 col-xs-6 tile tile_invoices" onclick="window.location='clientarea.php?action=invoices'">
            <a href="clientarea.php?action=invoices">
                <div class="icon">
                    <img class="icon-img" src="templates/ais/img/tile-invoices.svg" alt="Icon">
                    <i class="svg svg_ok-blue"></i>
                </div>
...
<form method="post" action="domainchecker.php" class="domain-register-panel">
    <div class="input-group input-group_padding field-registration">
        <input type="text" name="domain" class="form-control" placeholder="{$LANG.domainsearchplaceholder}">
        <div class="input-group-btn">
            <input type="submit" value="{$LANG.domainsregister}" class="btn btn-success">
            <input type="submit" name="transfer" value="{$LANG.domainstransfer}" class="btn btn-success btn-invert">
        </div>
    </div>
</form>
                    ...
                    {if $item->hasChildren()}
                        <div class="list-group{if $item->getChildrenAttribute('class')} {$item->getChildrenAttribute('class')}{/if}">
                            {foreach $item->getChildren() as $childItem}
                                ...
                                {else}
                                    <div menuItemName="{$childItem->getName()}" class="list-group-item{if $childItem->getClass()} {$childItem->getClass()}{/if}" id="{$childItem->getId()}" {if $item->getName() eq 'Active Products/Services' and  $childItem@iteration > 4}style="display: none"{/if}>
                                        {if $childItem->hasIcon()}<i class="{$childItem->getIcon()}"></i>&nbsp;{/if}
                                        {$childItem->getLabel()}
                                        {if $childItem->hasBadge()}&nbsp;<span class="badge">{$childItem->getBadge()}</span>{/if}
                                    </div>
